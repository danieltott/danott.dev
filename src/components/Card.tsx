...
Card.Cta = function CardCta({ children }: { children: React.ReactNode }) {
  return (
    <div
      aria-hidden="true"
      className="relative z-10 mt-4 flex items-center text-sm font-medium text-primary-500 md:text-base lg:text-lg"
    >
      {children}
...
Card.Eyebrow = function CardEyebrow<T extends React.ElementType = 'p'>({
  ...
  return (
    <Component
      className={clsx(
        className,
        'relative z-10 order-first mb-3 flex items-center gap-1 font-sans text-xs text-stone-400 dark:text-stone-500 md:text-sm',
        decorate && 'pl-3.5'
      )}
      {...props}
    >
